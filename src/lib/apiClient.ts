...
const BASE_URL = 'https://synex420-api-v3-20.onrender.com'
...
export const authApi = {
  ...
  register: (email: string, password: string, name: string, role?: string) => apiFetch<{ token: string }>('/api/auth/register', { method: 'POST', body: JSON.stringify({ email, password, name, role }) }),
...
export const patientApi = {
  ...
  update:   (id: string, data: any) => apiFetch(`/api/patients/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
};
...
export const nhifApi = {
  ...
  updateStatus:  (id: string, data: any) => apiFetch(`/api/nhif-claims/${id}/status`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const rpmApi = {
  ...
  resolveAlert: (id: string) => apiFetch(`/api/rpm-vitals/${id}`, { method: 'PATCH', body: JSON.stringify({ status: 'resolved' }) }),
};
