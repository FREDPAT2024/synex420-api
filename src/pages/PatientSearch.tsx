...
import { Search, User, Phone, MapPin, MoreVertical, Filter, ArrowUpDown, Shield, ChevronRight } from 'lucide-react';
...
export default function PatientSearch() {
  ...
  useEffect(() => {
    ...
    // Safety check: ensure user is authenticated before listening
    const setupListener = () => {
      setLoading(true);
      unsubscribe = patientService.listenToPatients((data) => {
        setPatients(data);
        setLoading(false);
      });
    };
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
                ...
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Identifiers</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Registration Status</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Contact Information</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Current Status</th>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-8 py-16 text-center text-slate-400 italic font-medium">Synchronizing records with facility database...</td>
                </tr>
              ) : filteredPatients.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-8 py-16 text-center text-slate-400 italic font-medium">No matching records found in the Master Patient Index.</td>
                </tr>
                ...
                <tr key={p.id} className="group hover:bg-slate-50 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 flex items-center gap-1 mt-1 font-medium">
                          <MapPin size={10} className="text-[#3B82F6]" /> {p.residence}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">OP: {p.opNo} • IP: {p.ipNo || 'N/A'}</p>
                    </div>
                  </td>
                  <td className="px-8 py-5">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter italic">REG: 2026</span>
                      </div>
                      {p.insuranceProvider && (
                        <div className="flex items-center gap-1 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                          <Shield size={10} className="text-[#3B82F6]" />
                          {p.insuranceProvider} COVERED
                        </div>
                      )}
                    </div>
                  </td>
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1 font-medium">
              Access to the Master Patient Index is logged. Every viewing of clinical records is strictly monitored per HIPAA/KMPDC regulations.
            </p>
          </div>
        </div>
